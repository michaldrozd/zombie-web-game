...
const MIN_NPC_SPAWN_DISTANCE_FROM_PLAYER = 300; // Minimum distance NPCs spawn from player at the start
...
export default class GameScene extends Phaser.Scene {
    ...
    init() {
        ...
        this.NPC_SPAWN_INTERVAL = 3500; // in milliseconds
    ...
    generateNPCs() {
        ...
        for (let i = 0; i < numNPCs; i++) {
            this.addNewNPCAtRandomPosition();
        }
    }
    ...
    addNewNPC() {
        ...
        while (!validPosition && attempts < maxAttempts) {
            ...
            if (!this.checkOverlapWithBuildings(x, y, 40, 40)) {
                validPosition = true;
            }
    ...
    /**
     * Adds a new NPC at a random position on the map, avoiding spawning too close to the player.
     */
    addNewNPCAtRandomPosition() {
        const maxAttempts = 100;
        let attempts = 0;
        let x, y;
        let validPosition = false;
        ...
        while (!validPosition && attempts < maxAttempts) {
            x = Phaser.Math.Between(0, this.GAME_WIDTH);
            y = Phaser.Math.Between(0, this.GAME_HEIGHT);

            const distanceToPlayer = Phaser.Math.Distance.Between(x, y, this.player.sprite.x, this.player.sprite.y);

            if (distanceToPlayer >= MIN_NPC_SPAWN_DISTANCE_FROM_PLAYER) {
                // Check overlap with buildings
                if (!this.checkOverlapWithBuildings(x, y, 40, 40)) {
                    validPosition = true;
                }
        ...
        if (validPosition) {
            ...
            console.warn('Could not place an NPC after maximum attempts.');
        }
    }
}
